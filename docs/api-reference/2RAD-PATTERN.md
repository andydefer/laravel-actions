...
```mermaid
flowchart TD
    subgraph FLOW ["FLUX D'EXÉCUTION 2RAD"]
        
        HTTP["🌐 HTTP Request"]
        
        REQUEST["📥 REQUEST (AbstractRequest)"]
        
        RECORD["📦 RECORD (AbstractRecord)"]
        
        ACTION["⚙️ ACTION (AbstractAction)"]
        
        DATA["📤 DATA (AbstractData)"]
        
        RESPONSE["✅ ResponseFactory → HTTP Response"]
        
        HTTP --> REQUEST
        REQUEST -->|produces| RECORD
        RECORD -->|passed to| ACTION
        ACTION -->|produces| DATA
        DATA --> RESPONSE
        
    end

    subgraph REQUEST_DETAIL ["🔍 DÉTAILS REQUEST"]
        R1["• Validation (rules)"]
        R2["• Authorization (authorize)"]
        R3["• Transformation (getRecord)"]
    end

    subgraph RECORD_DETAIL ["🔍 DÉTAILS RECORD"]
        RD1["• Type-safe DTO"]
        RD2["• Readonly properties"]
        RD3["• Immutable"]
    end

    subgraph ACTION_DETAIL ["🔍 DÉTAILS ACTION"]
        A1["• before() hook"]
        A2["• handle() business logic"]
        A3["• after() hook"]
        A4["• Returns ResponseFactory"]
    end

    subgraph DATA_DETAIL ["🔍 DÉTAILS DATA"]
        D1["• Response DTO"]
        D2["• Automatic camelCase"]
        D3["• Immutable"]
    end
    ...
    style FLOW fill:#1a1a2e,stroke:#16213e,color:#eee
    style HTTP fill:#e94560,stroke:#fff,color:#fff
    style REQUEST fill:#0f3460,stroke:#fff,color:#fff
    style RECORD fill:#0f3460,stroke:#fff,color:#fff
    style ACTION fill:#0f3460,stroke:#fff,color:#fff
    style DATA fill:#0f3460,stroke:#fff,color:#fff
    style RESPONSE fill:#4ecdc4,stroke:#fff,color:#1a1a2e
    
    style REQUEST_DETAIL fill:#1a2a3a,stroke:#4ecdc4,color:#eee
    style RECORD_DETAIL fill:#1a2a3a,stroke:#4ecdc4,color:#eee
    style ACTION_DETAIL fill:#1a2a3a,stroke:#4ecdc4,color:#eee
    style DATA_DETAIL fill:#1a2a3a,stroke:#4ecdc4,color:#eee
    
    style R1 fill:#2a3a4a,stroke:#4ecdc4,color:#fff
    style R2 fill:#2a3a4a,stroke:#4ecdc4,color:#fff
    style R3 fill:#2a3a4a,stroke:#4ecdc4,color:#fff
    style RD1 fill:#2a3a4a,stroke:#4ecdc4,color:#fff
    style RD2 fill:#2a3a4a,stroke:#4ecdc4,color:#fff
    style RD3 fill:#2a3a4a,stroke:#4ecdc4,color:#fff
    style A1 fill:#2a3a4a,stroke:#4ecdc4,color:#fff
    style A2 fill:#2a3a4a,stroke:#4ecdc4,color:#fff
    style A3 fill:#2a3a4a,stroke:#4ecdc4,color:#fff
    style A4 fill:#2a3a4a,stroke:#4ecdc4,color:#fff
    style D1 fill:#2a3a4a,stroke:#4ecdc4,color:#fff
    style D2 fill:#2a3a4a,stroke:#4ecdc4,color:#fff
    style D3 fill:#2a3a4a,stroke:#4ecdc4,color:#fff
```
